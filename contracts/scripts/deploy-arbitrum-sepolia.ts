// Arbitrum Sepolia Deployment Script with Gas Estimation
// This script:
// 1. Estimates gas costs before deployment
// 2. Deploys all contracts to Arbitrum Sepolia
// 3. Verifies contracts on Arbiscan
...
const ENTRYPOINT_ADDRESS = "0x0000000071727De22E5E9d8BAf0edAc6f37da032"; // ERC-4337 v0.7 (same across chains)
const USDC_ADDRESS = "0x75faf114eafb1BDbe2F0316DF893fd58CE46AA4d"; // Arbitrum Sepolia USDC
const PAYMASTER_INITIAL_FUNDING = ethers.parseEther("0.1"); // 0.1 ETH (Arbitrum has lower gas fees)
...
async function main() {
  console.log("\n╔════════════════════════════════════════════════════════════╗");
  console.log("║  Vlossom Protocol - Arbitrum Sepolia Deployment            ║");
  console.log("╚════════════════════════════════════════════════════════════╝\n");
  ...
  if (balance < ethers.parseEther("0.05")) {
    throw new Error("❌ Insufficient balance! Need at least 0.05 ETH. Get testnet ETH from: https://faucets.chain.link/arbitrum-sepolia");
  }
  ...
  console.log(`   npx hardhat verify --network arbitrum-sepolia ${factoryAddress} ${ENTRYPOINT_ADDRESS} ${deployer.address}`);
  console.log(`   npx hardhat verify --network arbitrum-sepolia ${paymasterAddress} ${ENTRYPOINT_ADDRESS} ${deployer.address}`);
  console.log(`   npx hardhat verify --network arbitrum-sepolia ${escrowAddress} ${USDC_ADDRESS} ${deployer.address} ${deployer.address}\n`);
  ...
  console.log(`   https://sepolia.arbiscan.io/address/${paymasterAddress}\n`);
  console.log("4. View Paymaster deposit:");
  console.log(`   Current: ${ethers.formatEther(paymasterBalance)} ETH`);
  console.log(`   Min threshold: ${process.env.PAYMASTER_MIN_BALANCE || "0.05"} ETH\n`);
}
